    ...
    return (
        <Layout>
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-5 py-8 md:py-16 mx-auto" style={{ width: '100%', maxWidth: '100vw' }}>
                        ...
                        <div key={index} className="mb-8">
                            ...
                            <div className="flex flex-wrap -m-4">
                                {product.filter(item => item.category === category).map((item, idx) => (
                                    <div onClick={() => window.location.href = `/productinfo/${item.id}`} key={idx} className="p-4 mb-4 mt-4 md:w-1/4 w-full">
                                        <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                            <div className="flex justify-center cursor-pointer">
                                                <img className="rounded-2xl w-full h-80 p-2 hover:scale-110 transition-scale-110 duration-300 ease-in-out object-cover" src={item.imageUrl} alt="product" />
                                            </div>
